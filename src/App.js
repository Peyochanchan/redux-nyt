  ...
  <div className="card d-flex flex-row justify-content-between">
    <div className="d-flex">
      ...
      <div className="d-flex flex-column" style={{ width: "75%" }}>
        ...
        <h5>
          <a href={url} target="_blank" rel="noreferrer" style={{ color: "#2980b9" }}>
            {title}
          </a>
        </h5>
        <i>{abstract}</i>
    ...
    >
      {links.map((link, index) => {
        return (
          ...
          >
            <a
              class={`nav-link ${link.name === active.name && "active" }`}
              href="#2"
              style={index === 0 ? styles.left : styles.right}>
          <i class={`fas fa-${link.icon}`}></i> {link.name}
            </a>
          </li>
        );
      })}
...
const List = ({ state: { active } }) => {
  ...
  return results.map((result) => <Card {...result}/>)
};
